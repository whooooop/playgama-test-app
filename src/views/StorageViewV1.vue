...
// The v1 storage API exposes per-type capability checks and a storage-type
// argument on every operation. These helpers stay defensive so the component
// degrades gracefully if rendered against a bridge that lacks the v1 shape.
const isLocalStorageSupported = computed(() => {
  const type = bridge.value?.STORAGE_TYPE?.LOCAL_STORAGE;
  if (!type || !bridge.value?.storage?.isSupported) return null;
  return bridge.value.storage.isSupported(type);
});

const isLocalStorageAvailable = computed(() => {
  const type = bridge.value?.STORAGE_TYPE?.LOCAL_STORAGE;
  if (!type || !bridge.value?.storage?.isAvailable) return null;
  return bridge.value.storage.isAvailable(type);
});

const isPlatformInternalSupported = computed(() => {
  const type = bridge.value?.STORAGE_TYPE?.PLATFORM_INTERNAL;
  if (!type || !bridge.value?.storage?.isSupported) return null;
  return bridge.value.storage.isSupported(type);
});
...
const defaultStorageType = computed(() => {
  return bridge.value?.storage?.defaultType ?? null;
});
